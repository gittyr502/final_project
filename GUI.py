...
class Camera(QMainWindow):
    ...
    def __init__(self,parent_class):
        ...
        # path to save
        self.save_path = r"C:\Users\1\Desktop\Efrat 2022\BOOTCAMP\PROJECT"
...
class PredictDialog(QDialog, QWidget):
    ...
    def predict(self):
        if self.is_first ==False:
            ...
            if((float)(values[0][:-1])>50.0):
                self.predict_label.setText(rf"Your image is:<br> {keys[0]}: {values[0]} <br> {keys[1]}: {values[1]} ")
            else:
                self.predict_label.setText(rf"We could not identify<br>try again<br> ")
