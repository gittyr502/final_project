...
def create_cifar10_classes_dict()->dict:
    ...
    :return cifar10 classes in format dict :
    '''
...
def create_cifar100_classes_dict() -> dict:
    '''
    creat dict of cifar100 classes
    :return: cifar100 classes in format dict
    '''
...
def create_df(dataset_dirname: str, files_list: List[str], labels: str):
    '''
     prepare df with all data to make perfect to save in csv
    :param dataset_dirname: cifar10 or cifar100
    :param files_list: list of batches
    :param labels: b'labels' or b'coarse_labels'
    :return: df to save to csv and array with images to save in resource
    '''
    df = pd.DataFrame()
    image_data = [[0] * 3072]
...
def save_image( data_arr, path_to_save):
    '''
    create image from nparray and save in path
    :param data_arr: arr in format 32*32*3
    :param path_to_save:
    :return: None
    '''
...
def save_images(df, data):
    ...
    :param df: contain all the data in csv
    :param data: images in nparray format
    :return: None
    '''
...
def choose_x_labels_from_cifar100(df, imgs, cifar100_source_new_labels:Dict, old_csv):
    '''
    add to data specific labels
    :param df: df with all data (like in csv file)
    :param imgs:
    :param cifar100_source_new_labels: dict with choosen labela from cifar100
    :param old_csv: csv that already dont contain new labels
    :return:
    '''
...
def insert_personal_image_to_csv (image_name:str, label:int ):
    ...
    :param image_name: name of personal image
...
def write_df_to_csv(df, newFile=False):
    ...
    :param df: with data to save to csv
    :param newFile: ia append or do new file
    :return: None
    '''
    if newFile:
...
def load_x_labels_from_cifar100(categorys_numbers: List[int]):
    ...
    :param categorys_numbers:
    :return:
    '''
    cifar100_source_labels=update_cifar100_dict_labels_to_zip(categorys_numbers)
    # print(cnfg.csv_path)
    old_csv = pd.DataFrame(pd.read_csv(r'C:\Users\1\Desktop\Efrat 2022\BOOTCAMP\PROJECT\cifar10_100.csv'))
...
def save_images_as_nparray():
    '''
    prepare data to zip to model
    :return: data to save on zip
    '''
...
def save_to_zip():
    '''
    split the data and save to zip file
    :return:None
    '''
...
def save_cifar10_dict_labels_to_zip():
    '''
    save cifar10 labels to zip so we can use after model
    :return: None
    '''
...
def update_cifar100_dict_labels_to_zip(labels_numbers_to_update:List[int]) :
    '''
    if add another clases from cifar100 we must update file
    :param labels_numbers_to_update: label that we add to data
...
def get_model_classes_dict():
    '''
    load and return zip file with all classes of cifar10&100
    :return zip file with classes:
    '''
